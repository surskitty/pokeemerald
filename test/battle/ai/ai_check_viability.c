...
AI_SINGLE_BATTLE_TEST("AI uses Worry Seed against Rest")
{
    u32 move;
    u64 aiFlags;
    PARAMETRIZE { move = MOVE_REST; aiFlags = AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT; }
    PARAMETRIZE { move = MOVE_FLY; aiFlags = AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT; }
    PARAMETRIZE { move = MOVE_REST; aiFlags = AI_FLAG_SMART_TRAINER | AI_FLAG_OMNISCIENT; }
    PARAMETRIZE { move = MOVE_FLY; aiFlags = AI_FLAG_SMART_TRAINER | AI_FLAG_OMNISCIENT; }
    GIVEN {
        AI_FLAGS(aiFlags);
        PLAYER(SPECIES_ZUBAT) { Moves(move, MOVE_SLEEP_TALK, MOVE_HEADBUTT); }
        OPPONENT(SPECIES_BUDEW) { Moves(MOVE_WORRY_SEED, MOVE_SLUDGE_BOMB, MOVE_GIGA_DRAIN); }
    } WHEN {
        if (move == MOVE_REST)
...
AI_SINGLE_BATTLE_TEST("AI uses Skill Swap against Poison Heal")
{
    u8 status;
    u64 aiFlags;
    PARAMETRIZE { status = STATUS1_POISON; aiFlags = AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT; }
    PARAMETRIZE { status = STATUS1_POISON; aiFlags = AI_FLAG_SMART_TRAINER; }
    PARAMETRIZE { status = STATUS1_TOXIC_POISON; aiFlags = AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT; }
    PARAMETRIZE { status = STATUS1_TOXIC_POISON; aiFlags = AI_FLAG_SMART_TRAINER; }
